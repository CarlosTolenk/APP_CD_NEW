...
class Home extends Component {
    ...
    async componentWillMount() {
        ...
        this.unsubscribe = await this.alerta.onSnapshot((querySnapshot) =>{
            let allAlertas = [];
            let alertaSingle
         ...
         this.unsubscribe = await this.planes.onSnapshot((querySnapshot) =>{
         let allPlanes = [];
          querySnapshot.forEach((doc) =>{
        ...
        this.unsubscribe = await this.eventos.onSnapshot((querySnapshot) =>{
        let allEventos = [];
          querySnapshot.forEach((doc) =>{
        ...
        this.unsubscribe = await this.loteria.onSnapshot((querySnapshot) =>{
        let allLoteria = [];
          querySnapshot.forEach((doc) =>{
        ...
        this.unsubscribe = await this.anuncios.onSnapshot((querySnapshot) =>{
            let allAnuncios = [];
            querySnapshot.forEach((doc) =>{
          ...
          this.unsubscribe = await this.numeros.onSnapshot((querySnapshot) =>{
            let AllNumeros = [];
            querySnapshot.forEach((doc) =>{
          ...
          this.unsubscribe = await this.lineaDirecta.onSnapshot((querySnapshot) => {
            let linea = []
            querySnapshot.forEach((doc) =>{
                linea.push(doc.data());
              });      
